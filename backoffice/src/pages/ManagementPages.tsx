...
export function PlansPage() {
  ...
  const remove = async () => { if (!removing?.id) return; try { await api.delete(`/business-plan/${removing.id}`); setRemoving(null); reload() } catch (cause) { setError(apiMessage(cause, t('genericError'))); setRemoving(null) } }
  const columns = useMemo<ColumnDef<BusinessPlan, unknown>[]>(() => [
    ...
    { id: 'actions', header: () => t('actions'), cell: ({ row }) => <div className="row-actions">{row.original.id && <Link to={`/plans/${row.original.id}/edit`}><Pencil size={16} /></Link>}<button className="danger-text" onClick={() => setRemoving(row.original)}><Trash2 size={16} /></button></div> },
  ], [t, i18n.language])
...
export function UsersPage() {
  ...
  const toggle = async () => { if (!confirming?.id) return; if (confirming.id === session.userId) { setError(t('selfDeactivate')); setConfirming(null); return } try { await api.put(`/user/${confirming.id}`, { ...confirming, password: undefined, enabled: !confirming.enabled }); setConfirming(null); reload() } catch (cause) { setError(apiMessage(cause, t('genericError'))) } }
  const columns = useMemo<ColumnDef<User, unknown>[]>(() => [
    { header: () => t('name'), accessorKey: 'name', cell: ({ row }) => <><b>{row.original.name || '—'}</b><small>{row.original.email}</small></> },
    { header: () => t('role'), accessorKey: 'role', cell: ({ row }) => <span className="badge blue">{row.original.role}</span> },
