...
export function DashboardPage() {
  ...
  const countOnly = { page: 0, pageSize: 1 }
  useEffect(() => { dispatch(loadTenants(countOnly)); dispatch(loadUsers(countOnly)); if (session.role === 'SysAdmin') dispatch(loadPlans(countOnly)) }, [dispatch, session.role])
  const cards = [{ label: t('activeTenants'), value: tenants.totalItems, icon: Building2, color: 'blue', to: '/tenants' }, { label: t('users'), value: users.totalItems, icon: Users, color: 'green', to: '/users' }, ...(session.role === 'SysAdmin' ? [{ label: t('availablePlans'), value: plans.totalItems, icon: CreditCard, color: 'yellow', to: '/plans' }] : [])]
