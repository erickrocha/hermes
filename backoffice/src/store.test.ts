...
describe('normalizeSession', () => {
  it('normalizes the backend camelCase token payload', () => {
    const session = normalizeSession({ accessToken: 'token', refreshToken: 'refresh', userId: 7, tenantId: 3, firstLogin: true, role: 'TenantOwner' })
    expect(session).toMatchObject({ accessToken: 'token', refreshToken: 'refresh', userId: 7, tenantId: 3, firstLogin: true, role: 'TenantOwner' })
  })
