...
use entity::customer_day_off_entity::{ActiveModel, Model};
use sea_orm::prelude::Date;
use sea_orm::{NotSet, Set};

/// `EPIC-SC-01-S02` (`HRMS-601`): a day `customer_id` is closed.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CustomerDayOff {
    pub id: Option<i64>,
...
pub struct CustomerDayOffEntityMapper {}

impl EntityMapper<CustomerDayOff, Model, ActiveModel> for CustomerDayOffEntityMapper {
    fn build_active_model(d: CustomerDayOff) -> ActiveModel {
        ActiveModel {
            id: match d.id {
                Some(id) => Set(id),
                None => NotSet,
    ...
    fn from_model(e: Model) -> CustomerDayOff {
        CustomerDayOff {
            id: Some(e.id),
    ...
    fn from_active_model(mut e: ActiveModel) -> CustomerDayOff {
        use sea_orm::TryIntoModel;
        let model: Result<Model, _> = e.clone().try_into_model();
        match model {
            Ok(m) => Self::from_model(m),
            Err(_) => CustomerDayOff {
                id: e.id.take(),
