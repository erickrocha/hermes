use crate::commons::entity_mapper::EntityMapper;
use crate::commons::functions::{bytes_para_string, string_to_bytes};
use crate::domain::enums::ChecklistType;
use chrono::NaiveDateTime;
use entity::checklist_run_entity::{ActiveModel, Model};
use sea_orm::{NotSet, Set};
use std::str::FromStr;

/// `EPIC-CK-03-S01` (`HRMS-652`): a submitted driver checklist. Opening and
/// closing a possession cycle are both this same shape -- only
/// `checklist_type` and `opening_checklist_id` differ between them.
#[derive(Debug, Clone, PartialEq)]
pub struct ChecklistRun {
    pub id: Option<i64>,
    pub uuid: Option<String>,
    pub tenant_id: Option<i64>,
    pub checklist_template_id: i64,
    pub driver_id: i64,
    pub vehicle_id: i64,
    pub checklist_type: ChecklistType,
    pub odometer_km: f64,
    pub notes: Option<String>,
    pub opening_checklist_id: Option<i64>,
    pub created_at: Option<NaiveDateTime>,
    pub created_by: Option<String>,
    pub updated_at: Option<NaiveDateTime>,
    pub updated_by: Option<String>,
}

pub struct ChecklistRunEntityMapper {}

impl EntityMapper<ChecklistRun, Model, ActiveModel> for ChecklistRunEntityMapper {
    fn build_active_model(d: ChecklistRun) -> ActiveModel {
        ActiveModel {
            id: match d.id {
                Some(id) => Set(id),
                None => NotSet,
            },
            uuid: match d.uuid {
                Some(uuid) => Set(string_to_bytes(&uuid)),
                None => NotSet,
            },
            tenant_id: Set(d.tenant_id),
            checklist_template_id: Set(d.checklist_template_id),
            driver_id: Set(d.driver_id),
            vehicle_id: Set(d.vehicle_id),
            checklist_type: Set(d.checklist_type.to_string()),
            odometer_km: Set(d.odometer_km),
            notes: Set(d.notes),
            opening_checklist_id: Set(d.opening_checklist_id),
            created_at: NotSet,
            created_by: NotSet,
            updated_at: NotSet,
            updated_by: NotSet,
        }
    }

    fn from_model(e: Model) -> ChecklistRun {
        ChecklistRun {
            id: Some(e.id),
            uuid: Some(bytes_para_string(e.uuid)),
            tenant_id: e.tenant_id,
            checklist_template_id: e.checklist_template_id,
            driver_id: e.driver_id,
            vehicle_id: e.vehicle_id,
            checklist_type: ChecklistType::from_str(&e.checklist_type)
                .unwrap_or(ChecklistType::Standalone),
            odometer_km: e.odometer_km,
            notes: e.notes,
            opening_checklist_id: e.opening_checklist_id,
            created_at: Some(e.created_at.naive_utc()),
            created_by: e.created_by,
            updated_at: Some(e.updated_at.naive_utc()),
            updated_by: e.updated_by,
        }
    }

    fn from_active_model(mut e: ActiveModel) -> ChecklistRun {
        use sea_orm::TryIntoModel;
        let model: Result<Model, _> = e.clone().try_into_model();
        match model {
            Ok(m) => Self::from_model(m),
            Err(_) => ChecklistRun {
                id: e.id.take(),
                uuid: e.uuid.take().map(bytes_para_string),
                tenant_id: e.tenant_id.take().flatten(),
                checklist_template_id: e.checklist_template_id.take().unwrap_or_default(),
                driver_id: e.driver_id.take().unwrap_or_default(),
                vehicle_id: e.vehicle_id.take().unwrap_or_default(),
                checklist_type: e
                    .checklist_type
                    .take()
                    .and_then(|value| ChecklistType::from_str(&value).ok())
                    .unwrap_or(ChecklistType::Standalone),
                odometer_km: e.odometer_km.take().unwrap_or_default(),
                notes: e.notes.take().flatten(),
                opening_checklist_id: e.opening_checklist_id.take().flatten(),
                created_at: e.created_at.take().map(|dt| dt.naive_utc()),
                created_by: e.created_by.take().flatten(),
                updated_at: e.updated_at.take().map(|dt| dt.naive_utc()),
                updated_by: e.updated_by.take().flatten(),
            },
        }
    }
}
